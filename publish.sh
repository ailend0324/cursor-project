...
case $TEMPLATE_TYPE in
    "travel")
        TEMPLATE_DIR="travel_blog"
        echo "🌍 使用旅行日记模板"
        ;;
    "investment")
        TEMPLATE_DIR="investment_notes"
        echo "💰 使用投资笔记模板"
        ;;
    "reading")
        TEMPLATE_DIR="reading_summary"
        echo "📚 使用读书分享模板"
        ;;
    "study")
        TEMPLATE_DIR="demo_new_page"
        echo "📖 使用学习笔记模板"
        ;;
    "professional")
        TEMPLATE_DIR="share_page"
        echo "💼 使用专业分析模板"
        ;;
    *)
...
case $TEMPLATE_TYPE in
    "travel")
        # 替换旅行模板的内容
        sed -i.bak "s/🌍 我的旅行日记/$PAGE_TITLE/g" "$TARGET_DIR/index.html"
        sed -i.bak "s/记录旅途中的美好时光和难忘经历/$PAGE_SUBTITLE/g" "$TARGET_DIR/index.html"
        
        # 替换核心亮点
        if [ ! -z "$HIGHLIGHT_CONTENT" ]; then
            # 转换换行符为HTML
            HIGHLIGHT_HTML=$(echo "$HIGHLIGHT_CONTENT" | sed 's/$/\\n/g' | tr -d '\n' | sed 's/\\n/<br>/g')
            sed -i.bak "s/这次去了美丽的三亚，感受到了海南的热带风情。白色的沙滩、碧蓝的海水，还有当地的椰子鸡火锅，每一个瞬间都让人难忘！/$HIGHLIGHT_HTML/g" "$TARGET_DIR/index.html"
        fi
        ;;
        
    "investment")
        # 替换投资模板的内容
        sed -i.bak "s/💰 我的投资学习笔记/$PAGE_TITLE/g" "$TARGET_DIR/index.html"
        sed -i.bak "s/记录投资路上的学习心得和实战经验/$PAGE_SUBTITLE/g" "$TARGET_DIR/index.html"
        ;;
        
    "reading")
        # 替换读书模板的内容
        sed -i.bak "s/📚 读书分享/$PAGE_TITLE/g" "$TARGET_DIR/index.html"
        sed -i.bak "s/分享读书心得，记录思考感悟/$PAGE_SUBTITLE/g" "$TARGET_DIR/index.html"
        ;;
        
    "study")
        # 替换学习模板的内容
        sed -i.bak "s/📖 我的学习笔记分享/$PAGE_TITLE/g" "$TARGET_DIR/index.html"
        sed -i.bak "s/记录学习过程中的重要知识点和心得体会/$PAGE_SUBTITLE/g" "$TARGET_DIR/index.html"
        ;;
esac
...
sed -i.bak "s/2025年7月22日/$CURRENT_DATE/g" "$TARGET_DIR/index.html"
rm -f "$TARGET_DIR/index.html.bak"

echo "✅ 页面生成完成: $TARGET_DIR/index.html"
